...
async def ingest_cricsheet_csv(csv_path: str):
    # Read CSV with proper handling of nested fields
    df = pd.read_csv(csv_path, low_memory=False)

    # Extract match metadata
    match_info = {
        "match_id": df["info__match_type_number"].iloc[0],
        ...
        "venue_id": ObjectId(),  # Temporary ObjectId, updated later
        "teams": [
            {
                "team_id": ObjectId(),
                "score": None,  # Computed later
                "wickets": None,
                "overs": None,
            },
            {"team_id": ObjectId(), "score": None, "wickets": None, "overs": None},
        ],
        "toss": {
            "winner_team_id": None,  # Updated later
    ...
    for i, team_name in enumerate(teams):
        team_doc = {
            ...
            "name": team_name,
            "country": team_name,  # Simplified
            "created_at": get_current_timestamp(),
            "updated_at": get_current_timestamp(),
        }
        await db.db["teams"].insert_one(team_doc)
        team_map[team_name] = team_doc["team_id"]
        match_info["teams"][i]["team_id"] = team_doc["team_id"]

    # Update toss and result with team IDs
    ...
    # Venue
    venue_doc = {
        "venue_id": str(match_info["venue_id"]),
        "name": df["info__venue"].iloc[0],
        "city": None,  # Not in CSV, can be enriched later
        "country": "Australia" if "Australia" in df["info__venue"].iloc[0] else None,
        "pitch_type": None,
        "average_scores": {},
        "toss_stats": {},
        "created_at": get_current_timestamp(),
    ...
    match_info["venue_id"] = venue_doc["venue_id"]

    # Players
    player_map = {}
    for team in ["Australia", "Sri Lanka"]:
    ...
    for team_name, team_data in innings:
        ...
        wickets = (
            team_data["innings__overs__deliveries__wickets__player_out"].notna().sum()
        )
        overs = (
            team_data["innings__overs__over"].max()
            + (team_data["innings__overs__deliveries__batter"].count() % 6) / 10
        )
        ...
        for batter_name, batter_data in batters:
            ...
            performance_doc = {
                ...
                "batting": {
                    ...
                    "dismissal": (
                        batter_data["innings__overs__deliveries__wickets__kind"].iloc[
                            -1
                        ]
                        if dismissals > 0
                        else None
                ...
                "bowling": None,  # Computed separately
                "created_at": get_current_timestamp(),
                "updated_at": get_current_timestamp(),
            }
            await db.db["playerPerformances"].insert_one(performance_doc)

        # Bowlers (from opposing team's innings)
    ...
    # Analytics (Placeholder for collapses)
    collapses = []
    for team_name, team_data in innings:
